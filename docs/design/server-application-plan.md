...
단, **지금(M0~M1) 서버를 만들지 않는다.** 이 문서의 1차 산출물은 "서버 없이 개발하되 위 4개를 꽂을 소켓(계약)을 지키는 것"(§7)이고, 실제 서버는 §12의 단계별 도입 로드맵 순서로 들어온다.
...
서버가 소유하는 데이터(= 핵 방지의 대상). **원인을 저장하고 결과는 재계산**하는 세이브 원칙(정본 §1 개요·목적의 "원인 저장" 원칙)이 서버에서도 그대로 유효하다.
...
서버는 한 번에 세우지 않는다. **각 단계는 독립적으로 가치를 내는 세로 슬라이스**이며, 앞 단계 없이 뒤 단계를 시작하지 않는다. 마일스톤(§관련 기획서 로드맵)과 나란히 가는 **별도 기술 트랙**이다.

| 단계 | 도입 내용 | 선행 조건·시점 | 이 순서인 이유 |
|:---:|-----------|----------------|----------------|
| **0** | 서버 없음. 로컬 파일 세이브(`FileSaveRepository`)로 개발. §7 계약만 준수 | **지금 (M0~M1)** | 코어 루프 검증은 로컬로 가능. 서버를 먼저 세우면 검증할 게임이 없다 |
| **1** | 계정(익명 디바이스) + `ServerSaveRepository` + **서버 시간 기준** | M1 언저리 | 유저·세이브가 쌓이기 전에 권위 저장으로 이관해야 값싸다. 서버 시간이 오프라인 보상 시계 조작을 차단(§6.1) |
| **2** | presence + 채팅(월드·사냥터) — 하나의 WebSocket에 멀티플렉싱. `RemotePlayerView` | presence·채팅 스파이크 후(M1 종료 전후) | 사냥터 룸 = 사냥터 채팅 스코프가 같아 함께 검증한다(§6.2). 존재 공유는 게임 정체성이라 조기 검증 가치가 크다 |
| **3** | 파티·길드(영속 소셜, DB) + **파밍 획득률 서버 검증 활성화**(§6.1) | M2~M3 진행 중 | 파티·길드는 릴레이가 아닌 영속 상태라 백엔드 소셜 기능이 필요. 재화가 의미를 갖기 시작하면 검증을 켠다 |
| **4** | **파티 보스전** — 레이드 룸(보스만 서버 소유 + 로컬 판정, §6.4) | **모험·성장 등 코어 시스템 완비 후** | 실시간 매치 룸 인프라의 **첫 실전**. 협동이라 지연에 관대해 안전하다. M3에서 만든 예고형 보스 패턴(§7)을 서버 재생으로 재사용 |
| **5** | **실시간 PvP**(서버 권위 틱 + 클라 예측·보간·판정 보상, §6.3) + 랭킹 | **최후순위** (4단계 이후) | 가장 비싼 넷코드. 4단계에서 검증된 룸 인프라 위에서 시작한다. 손맛(M3 능동 전투)이 먼저 완성돼 있어야 한다 |

> **핵심**: 4·5단계(실시간 매치)는 사용자 확정대로 **모험·성장이 다 갖춰진 뒤**에 온다. 그 전(0~3단계)은 PvP 전용 작업이 아니라, 세이브·존재 공유·소셜처럼 **그 자체로 게임에 필요한** 인프라다. PvP는 이 인프라가 다 깔린 위에 얹히는 마지막 조각이다.
