...
> **최종 갱신**: 2026-07-22 · **관련 기획서**: (이 문서가 최상위) · [characters-and-companions.md](./characters-and-companions.md)
...
> **이 진단은 2026-07-11 시점의 기록이다.** 이후 M0(§5.2)에서 스포너·골드·세이브·`GameManager`가 차례로 해소됐다. 진단을 지우지 않고 남기는 이유는, 로드맵의 우선순위가 **어떤 문제에서 파생됐는지**가 보이지 않으면 순서의 근거도 사라지기 때문이다.

### 2.1 시스템은 두껍고, 게임은 비어 있다

| 층위 | 상태 | 근거 |
|------|------|------|
| **시스템(엔진)** | 과잉에 가깝게 충분 | 상태 머신 5종 · `StatType` 20종 · 3레이어 모디파이어 · 스킬 6슬롯 + 쿨다운 + 캐스트 게이트 · 자동전투 AI · `PlayerInputRouter`의 Active↔Idle 런타임 스왑 |
| **콘텐츠(게임)** | 사실상 0 | 스킬 1개(`BasicAttack`) · 버프 1개(`PowerUp`) · 장비 1개(`Dragon_Hat`) · 적 스탯 1개 · 씬 1개(`Dev/SampleScene`) |
| **게임 루프** | **닫히지 않음**(진단 시점) | 스포너 없음 · 골드 없음 · 드랍 없음 · 세이브 없음 · 오프라인 보상 없음 · `GameManager`는 빈 스텁 |

### 2.2 가장 치명적인 결함 — 방치할 수 없는 방치형

`EnemyUnit.Die()`는 처치 보상(경험치)을 발행한 뒤 적을 `SetActive(false)` 처리한다. **적을 다시 등장시키는 주체가 코드 어디에도 없다.**
...
### 2.3 ~~두 번째 치명 결함 — 레벨을 올려도 강해지지 않는다~~ **해소(2026-07-21, 커밋 58906bd)**

당시 `PlayerBaseStatResolver.Resolve(progressionState, config)`는 레벨 정보(`progressionState`)를 파라미터로 받고도 사용하지 않아, **레벨이 1이든 100이든 스탯이 동일했다.** 현재는 `PlayerLevelTable`(SO)을 도입하고 계약을 `Resolve(state)`로 바꿔 레벨이 베이스 스탯에 실제 반영된다(상세: [base-stat-resolver-level-scaling.md](../reports/base-stat-resolver-level-scaling.md)).

경험치 공급원(`EnemyKillReward` → `IExpReceiver`) 배선에 이어 이 구간까지 닫혀, **"적 처치 → 경험치 → 레벨업 → 더 강해짐"의 인과는 이제 끝까지 이어진다.** 남은 끊김은 §2.2(스포너)와 재화·드랍이다.
...
flowchart LR
    ...
    Exp -->|"구현됨"| Level["레벨업"]
    Level -->|"구현됨 (2026-07-21)"| Power["더 강해짐"]
    ...
    style Power fill:#2d5016,color:#fff
    style Loot fill:#5c1a1a,color:#fff
```

초록은 구현됨, 빨강은 부재. 리졸버 구간(§2.3)은 닫혔지만 **스포너·재화가 끊겨 있어 사이클은 여전히 한 바퀴도 돌지 않는다.** 남은 두 지점(스포너 · 재화)이 정확히 M0의 잔여 범위다.
...
| **멀티 상세·서버 방향** (07-22) | 지향점은 **"메이플 키우기"류** — 사냥터를 공유하되 몬스터·스킬 이펙트는 비공유, **위치와 스킬 모션을 공유**한다(§3.5 존재 공유의 구체화). **채팅 필수**: 월드·사냥터·파티·길드 채널을 모든 위치에서 제공. **PvP는 후순위일 뿐 반드시 구현**하며, **핵(치팅) 방지도 필수**다 | 서버 권위(캐릭터 시트·재화·PvP 결과를 서버 소유)가 조건부 옵션이 아니라 **확정된 종착점**이 된다. 전투는 클라 로컬 유지(몬스터 비공유), 검증은 획득률 상한 방식. 상세: [server-application-plan.md](../design/server-application-plan.md) |
| **PvP 형태·시점 · 파티 보스전** (07-22) | PvP는 **"손맛 있는 실시간 전투"**(조작·회피·타이밍이 승부를 가르는 룸 매치)로 확정 — 비동기 오토배틀이 아니다. 착수는 **모험·성장 등 코어 시스템 완비 후**(M4 이후 최후순위). **파티 보스전(협동 레이드)도 필수**이며, 멀티 전투는 **파티 보스전 → 실시간 PvP** 순으로 쌓는다 | 실시간 매치 룸 인프라(서버 권위 틱 + 클라 예측)가 최후순위 트랙으로 추가된다. 파티 보스전은 "보스만 서버 소유 + 로컬 판정"이라 지연에 관대해 PvP의 징검다리가 된다. **M3의 보스 패턴은 예고형 타임라인 데이터로 설계**해야 솔로 보스가 파티 보스로 재사용된다. 상세: [server-application-plan.md](../design/server-application-plan.md) §6.3·§6.4 |
...
| 구분 | 내용 |
|------|------|
| **포함** | ~~**① 레벨→스탯 리졸버**~~(**완료 2026-07-21**) · ~~**② 적 스포너**(지속 스폰 + 오브젝트 풀링)~~(**완료 2026-07-24**) · ~~**③ 골드 재화**(`Wallet`) + 적 처치 골드 보상~~(**완료 2026-07-26**) · ~~**④ 세이브/로드**~~(**완료 2026-08-23**) · ~~**⑤ `GameManager` 실체화**(게임 루프 소유)~~(**완료 2026-08-23**) |
...
> **진행(2026-08-23)**: ①~⑤ **구현 완료**. DoD의 런타임 실측(10분 방치)과 씬 배선(`GameManager` 배치)이 남았다. 구현 상세는 [m0-close-the-loop-plan.md](../design/m0-close-the-loop-plan.md), 세이브 as-built는 [specs/core/save.md](../specs/core/save.md) 참조.
...
| 구분 | 내용 |
|------|------|
| **포함** | 보스 스테이지(자동전투로는 뚫리지 않는 설계) · 능동 모드 전환 진입점 · 보스 공격 패턴(`EnemyAttacker` 확장) · 회피/타이밍 요소 · **보스 패턴은 예고형 타임라인 데이터로 설계**(§3.5 — 파티 보스전 재사용 전제) |
| **미포함** | PvP(§3.5 — **실시간 손맛 전투로 필수 구현**, 코어 시스템 완비 후) · 파티 보스전(§3.5 — 필수, PvP보다 먼저 구축하는 멀티 트랙) · 랭킹 |
...
**해소 완료** (2026-07-22, §3.5): **멀티 상세·서버 방향** — 메이플 키우기류 존재 공유(위치·스킬 모션), 채팅 채널 필수, **PvP 필수(후순위)**, 핵 방지 필수 → 서버 권위 종착점 확정([server-application-plan.md](../design/server-application-plan.md)).
**해소 완료** (2026-07-22, §3.5): **PvP 형태·시점 · 파티 보스전** — PvP는 실시간 손맛 전투(코어 시스템 완비 후 착수), 파티 보스전은 "보스만 서버 소유 + 로컬 판정" 방식으로 필수, 구축 순서는 레이드 → PvP. M3 보스 패턴은 예고형 타임라인 데이터로.
